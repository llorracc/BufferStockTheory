...
\begin{equation}\label{eq:veqnNrmRecBellman}
  \begin{split}
    ...
    \\ {\kNrm}_{t+1} & = \aNrm_{t}/\Rnd{\PermGroFac}_{t+1} %= ~  \RNrm_{t+1}\aNrm_{t}
    \\ {\bNrm}_{t+1} & = \kNrm_{t+1} \Rfree %\Rnd{\PermGroFac}_{t+1} %= ~  \RNrm_{t+1}\aNrm_{t}
    \\ \mNrm_{t+1}  & = ~ \bNrm_{t+1} +\tranShkAll_{t+1} .
                      %       \end{align*}
  \end{split}  \tag{$\mathscr{P}_{N}$}
\end{equation}
%where $\RNrm_{t+1}\colon = (\Rfree/\PermGroFac_{t+1})$ is a `permanent-income-growth-normalized' return factor.
...
% FVAC is really about conusmption paths along path of strictly positive income shocks? Explain this somwhere in the paper.
...
% AS: This is more a definition than an assumption.  Should we have a set of definitions distinct from the assumptions?
...
\begin{align*}%  \label{eq:limitlowerupper}
  \lim_{m \rightarrow \infty} \bar{\cFunc}(\mNrm)/\ushort{\cFunc}(\mNrm)
  & = \lim_{m \rightarrow \infty} (\mNrm -1+ \hNrm)/\mNrm  \\
  & = 1,
\end{align*}
so as $\mNrm \rightarrow \infty, \usual{\cFunc}(\mNrm)/\ushort{\cFunc}(\mNrm) \rightarrow 1$, and the continuous differentiability and strict concavity of $\usual{\cFunc}(\mNrm)$ therefore implies
...
\begin{comment}
  ...
  \end{align}
  \begin{align*}
    \CLvlAgg_{t}  % & l= \bar{\tilde{\cNrm}}_{t} = \Harm{\Mean}_{t+1}[\cNrm_{t}\permLvl_{t}] = \Harm{\Mean}_{t+1}[\cNrm_{t}]\Harm{\Mean}_{t+1}[\permLvl_{t}] + \cov_{t}(\cNrm_{t}, \permLvl_{t}) \\
                 & = %\bar{\tilde{\cNrm}}_{t} =
                   \Harm{\Mean}_{t\phantom{+1}}[\cNrm]\phantom{\PermGroFac} = \Mean[\cNrm_{t\phantom{t+1}}]\phantom{\PermGroFac} + \cov_{t}(\cNrm_{t}, \permLvl_{t})  \\
    \CLvlAgg_{t+1} & = \Harm{\Mean}_{t         +1 }[\cNrm]         \PermGroFac  = \cNrmAvg_{t+1}\PermGroFac  + \cov_{t+1}(\cNrm_{t+1}, \permLvl_{t+1})
  \end{align*}
...
\begin{comment}
  ...
  \begin{equation}
    \begin{split}\label{eq:covProblem}
      \CLvlAgg_{t+1} & = \PermGroFac \CLvlAgg_{t}\notag \\
...
\input{\TableDir/Required}\reversemarginpar\marginpar{\tiny\textcolor{orange}{\sout{AS to CDC: Can you please make sure the first line of Sec. refs in the Freidman/Muth row are OK with you?}}}
