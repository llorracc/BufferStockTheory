...
\marginpar{\tiny \textcolor{orange}{See the discussion in section \textcolor{orange}{[wherever]} for further elaboration.}}
...
\begin{comment}
% AS to AS: I think we have addressed this so removed margin par.
...
For example, if $\PermGroFac=0$, the problem is a standard `cake-eating' problem with a nondegenerate solution under \hyperlink{RIC}{return impatience}.% chktex 10 cf. Stachurski and Ma JET 2021 claim of neccessity of FVAC. 
...
\begin{comment}
AS to AS: improve the intuition above.
Incorporate some of the ideas about the rate of market resources and return impatience from the text below?
...
% AS TO AS: check RIC is the "stronger assumption in final math pass. 19072024. Also in the appendix say that the opertor is eventually contracting when RIC does hold. That is, RIC does not resolve the problem of remark2 
...
\begin{comment}
AS to AS: Make sure the use of the provide command for
 the fig file below is correct!
\end{comment}

\marginpar{\tiny{\textcolor{orange}{AS to CDC.
Chris, is the "\\providecommand" used properly in the figure?}}}
...
The quantitative credibility of the argument depends on the model's match to the distribution of permanent income inequality, which would not be possible in a model without a nondegenerate steady-state distribution of permanent income.
...
% \bibstandalone{BufferStockTheory}
