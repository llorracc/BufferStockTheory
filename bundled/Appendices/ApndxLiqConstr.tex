...
$\bNrm_{t}=0$:
\begin{equation}\begin{gathered}\begin{aligned}
  \bNrm_{\#}^{n} & =  \mathbb{C}_{t-n}^{t} - \overbrace{\left(\frac{1-\RNrmByGRnd^{-(n+1)}}{1-\RNrmByGRnd^{-1}}\right)}^{\hNrm_{\#}^{n}} \label{eq:bPound}.
\end{aligned}\end{gathered}\end{equation}
...
The \FHWC~requires $\RNrmByGRnd^{-1} < 1$, in which case the second term in~\eqref{\localorexternallabel{eq:bPound}} limits to a constant as $n \uparrow \infty$, and~\eqref{\localorexternallabel{eq:bToInfty}} reduces to a requirement that
\begin{eqnarray*}
...
\begin{equation}\begin{gathered}\begin{aligned}
  ...
  \\   \lim_{n \rightarrow \infty} \left(\frac{\RPFac}{\RPFac-1}-\frac{\RNrmByGRnd^{-1}}{\RNrmByGRnd^{-1}-1}\right)\RNrmByGRnd^{-n}-\left(\frac{\GPFacRaw^{-n}}{\RPFac-1}\right)  & = \infty \label{eq:FHWCfails} 
...
\end{eqnarray}
which makes clear that with $\cncl{\FHWC} \Rightarrow \RNrmByGRnd^{-1} > 1$ and $\cncl{\RIC} \Rightarrow \RPFac > 1$ the numerators and denominators of both terms multiplying $\RNrmByGRnd^{-n}$ can be seen transparently to be positive.  So, the terms multiplying
$\RNrmByGRnd^{-n}$ in~\eqref{\localorexternallabel{eq:FHWCfails}} will be positive if
\begin{eqnarray*}
...
\begin{comment}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
    \lim_{n \rightarrow \infty} \RNrmByGRnd^{n}\bNrm_{\#}^{n}  & =  \left(\frac{-\RPFac}{1-\RPFac}\right) - \RNrmByGRnd^{n}\left(\frac{1-\RNrmByGRnd^{-1}\RNrmByGRnd^{-n}}{1-\RNrmByGRnd^{-1}}\right) %\label{eq:bPoundLim}
