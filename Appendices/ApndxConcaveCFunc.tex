...
First we show that if \hyperlink{FHWC}{finite limiting human wealth} (Assumption \ref{ass:FHWC}) and \hyperlink{GICRaw}{growth impatience} (Assumption \ref{ass:GICRaw}) are both satisfied, \hyperlink{PFFVAC}{perfect foresight finite value of autarky} (Equation \eqref{eq:PFFVAC}) holds.
...
Let $t<T$ and suppose $\pZero>0$ (Assumption~\ref{ass:shocks}).
...
%% AS to AS: The domain of the argument of the functions below is only stated the first three equations. This should be fine since the domains are the same in the later equations, but may cause confusion. 
...
As such, there exists $k$ such that $\pZero \DiscFac {(\Rfree (1-\MPCmax_{T-k}))}^{1-\CRRA}<1$ and Equation \eqref{eq:MPCMAXKleq1} holds for all $\MPCmaxInf\leq \MPCmax_{T-k}$.
...
\end{equation}
%
a non-empty interval because $\pZero \DiscFac {(\Rfree (1-\MPCmax_{k}))}^{1-\CRRA}<1$ (by \hyperlink{WRIC}{weak return impatience} via Claim \ref{claim:MPCMAXKleq1}) and $\beta\Ex\PermGroFacRnd^{1-\CRRA}<1$ (by \hyperlink{FVAC}{finite value of autarky}, Assumption \ref{ass:FVAC}).
...
\begin{equation}\begin{gathered}\begin{aligned}
  \TMap^{\MPCminInf, \MPCmaxInf}{\mathfrak{\xFunc}}(\mNrm_{t}) & = \underset{\cNrm_{t} \in
                                            ...
                                            \left( {\mNrm}_{t+1}\right) \right] \right\}, \qquad \mNrm_{t}\in \Reals_{++}  %%% \label{eq:condition1}
\end{aligned}\end{gathered}\end{equation}
%
...
As such, define $\Shrinker$ by Equation \eqref{eq:shnkrdef} and note that $\pZero \DiscFac {(\Rfree (1-\MPCmax_{k}))}^{1-\CRRA} < \alpha < 1$ and $\alpha> \beta\Ex\PermGroFacRnd^{1-\CRRA}$.
...
\subsection{Properties of the Converged Consumption Function}\label{subsec:Properties}
...
Since each $\cFunc_{T-n}$ is concave with marginal propensity to consume no greater than $\MPCmax_{T-n}\leq 1$, the family is uniformly Lipschitz, so $\cFunc_{T-n+1}\rightarrow \cFunc$ uniformly on compact subsets of $\Reals_{++}$ (Theorem 10.8 in \cite{Rockafellar1972}); combined with the almost-sure convergence of $\mNrm_{t+1}$ this gives almost-sure convergence of the integrand.
Making use of the Dominated Convergence Theorem (see proof of Claim \ref{clm:hiraguchi_cont}), $\Rfree \DiscFac \Ex_{t}[ \Rnd{\PermGroFac}_{t+1}^{-\CRRA} \cFunc_{T-n+1}(\mNrm_{t+1})^{-\CRRA}]$ converges to $ \Rfree \DiscFac \Ex_{t}[ \Rnd{\PermGroFac}_{t+1}^{-\CRRA} \cFunc(\mNrm^{\nxt})^{-\CRRA}]$.
...
\end{aligned}\end{gathered}\end{equation}
%
Re-arranging the terms above: if \hyperlink{RIC}{return impatience} (Assumption \ref{ass:RIC}) holds then $\MPCminmin^{-\CRRA}>0$ may be divided out, giving $\MPCminmin =1-  \APFac/\Rfree$; if it fails, $\MPCmin_{T-n}\rightarrow 0$ (Lemma \ref{lemm:MPC}) passes to the limit, giving $\MPCminmin=0$. In either case $\MPCminmin=\max\{0,1-\APFac/\Rfree\}=\MPCmin$ as required.
