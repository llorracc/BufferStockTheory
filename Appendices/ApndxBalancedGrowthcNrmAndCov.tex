...
%% AS TO AS: TRIPLE CHECK THIS CLAIM IN FINAL PASS 
...
\begin{claim}\label{claim:gicimpliesharmimp}
...
\begin{remark}
The threshold in \eqref{eq:HarmImp} is sharp: subtracting $\log\PermGroFac = \log(\PermGroFac\,\Ex[\permShk])$ and using $\Ex[\permShk]=1$, \eqref{eq:HarmImp} is equivalent to
\begin{equation*}
  \log \GPFacRaw < \Ex[\permShk\log\permShk] = D_{\mathrm{KL}}(\permShk f \,\Vert\, f) \geq 0 ,
\end{equation*}
the Kullback--Leibler divergence between the permanent-income-weighted density $\permShk f$ and $f$. Since this divergence is nonnegative (strictly positive whenever $\permShk$ is nondegenerate), \hyperlink{GICRaw}{growth impatience} ($\log\GPFacRaw<0$) is sufficient but stronger than necessary; permanent-income risk relaxes the requirement.
\end{remark}
...
\begin{figure}[!htb]
  \centerline{
    \includegraphics[width=0.9\textwidth]{\FigDir/logcNrm}
  }
...
\begin{figure}[!htb]
  \centerline{
    \includegraphics[width=0.9\textwidth]{\FigDir/logcov}
  }
