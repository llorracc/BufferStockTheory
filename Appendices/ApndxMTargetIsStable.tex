...
\begin{proposition}\label{prop:convgGrowth}\hypertarget{LimitsAsmtToZero}{}
  Suppose the non-degenerate converged consumption function $\cFunc$ exists (Assumption~\ref{ass:shocks}). Then $\lim\limits_{\mNrm_{t} \rightarrow  0} \Ex_{t}[\cLvl_{t+1}/\cLvl_{t}] =  \infty$, and, if in addition \hyperlink{RIC}{return impatience} (Assumption~\ref{ass:RIC}) holds, $\lim\limits_{\mNrm_{t} \rightarrow \infty} \Ex_{t}[\cLvl_{t+1}/\cLvl_{t}] =  {\APFac}$.
\end{proposition}
...
\compilingassubfile{\setcounter{theorem}{1}}
  ...
  \end{lemma}
  \begin{proof}
  Define $\Phi_{R}(\mNrm)=R\,\aFunc(\mNrm)+1-\mNrm$ with $\aFunc(\mNrm)=\mNrm-\cFunc(\mNrm)$, so Equations~\eqref{eq:balgrostable} and~\eqref{eq:mTargImplicit} read $\Phi_{\RNrmByG}(\BalGroFac{\mNrm})=0$ and $\Phi_{\bar{\RNrmByGRnd}}(\TargetNrm{\mNrm})=0$, where $\bar{\RNrmByGRnd}=\RNrmByG\,\Ex_{t}[\permShk^{-1}]\geq\RNrmByG$ by Jensen ($\Ex_{t}[\permShk]=1$).
  ...
  As $\Phi_{\bar{\RNrmByGRnd}}$ is strictly decreasing with unique root $\TargetNrm{\mNrm}$ (proof of Theorem~\ref{thm:target}), this forces $\BalGroFac{\mNrm}\leq\TargetNrm{\mNrm}$.
  \end{proof}
...
\subsubsection{Existence of Individual Buffer Stock Target}\label{subsubsec:AppxIndividTarget} %%% \label{subsubsec:AppxIndividTarget}
...
%% AS to AS: Should we explicitly define \grave{\mNrm}_{t+1}? Non-critical. 
...
%% AS TO AS: Triple check the eq. refs below. Non-critical. 
...
%% AS to AS: Show this next point explicitly using algebra and limits. It may be useful. Non-critical. 
...
%% AS TO AS: terms like {\frac{\APFac}{\Rfree}}^{{\RPFac}} do not have a t+1 subscript on the permshock. This is a slight inconsistency (but technically fine). Non critical. 
...
%% AS TO AS: The below is correct, but maybe include steps later. Non-crtical. The steps below are not symmetric to the steps in the proof above. 
...
where the last two lines are merely a restatement of \hyperlink{GICRaw}{growth impatience}.
...
Notice  that $\frac{\Delta_{\epsilon}\cFunc({\mNrm}_t)}{\epsilon} \leq \frac{\cFunc(\mNrm_{t})}{\mNrm_{t}}<1$ since $\frac{\cFunc(\mNrm_{t})}{\mNrm_{t}}$ is decreasing in $\mNrm_{t}$ by Claim \ref{claim:rationondec} in Appendix.
...
Since $\cFunc$ is concave with $\lim\limits_{\mNrm\rightarrow\infty}\cFunc^{\prime}(\mNrm)=\MPCmin$ (Lemma \ref{lemma:MPCBoundsConvg}), $\frac{\Delta_{\epsilon}\cFunc(\mNrm_{t})}{\epsilon}=\cFunc^{\prime}(\xi)\geq\MPCmin$ by the Mean Value Theorem.
It follows that:
%
\begin{align*}
...
\end{align*}
%
which is negative because \hyperlink{GICRaw}{growth impatience} says $\GPFacRaw < 1$.
Conversely, when \hyperlink{RIC}{return impatience} holds fails, recall $\lim\limits_{\mNrm_{t} \rightarrow \infty} \frac{\cFunc(\mNrm_{t})}{\mNrm_{t}} = 0$.
...
\end{equation*}
Combined with strict monotonicity of $\difFunc$ and $\difFunc(\mBalLvl)=0$, $\mNrm_t\lessgtr\mBalLvl$ gives $\Ex_{t}[\permShk_{t+1}\mNrm_{t+1}/\mNrm_{t}]\gtrless1$, hence \eqref{eq:stabilityStE}.
