...
\input{\econtexRoot/@local/dir-paths}\usepackage{econark-ifsubfile}\usepackage{econark-xrsetup}
\compilingasstandalone{\externaldocument{\LaTeXGenerated/BufferStockTheory}\providecommand{\texname}{}\renewcommand{\texname}{Introduction}} % Get xrefs -- esp to appendix -- from main file; only works properly if main file has already been compiled;
...
% (Indeed, Appendix~\ref{sec:CIsTwiceDifferentiable} shows that $\cFunc(\mNrm)$ is not just continuous, but twice continuously differentiable.)
...
Concavity of the consumption function means that $\cFunc^{\prime}$ is a decreasing function, so everywhere 
\begin{align*}
  \RNrmByGRnd\left(1-\cFunc^{\prime}({\mNrm}_t)\right) & < \RNrmByGRnd
\end{align*}
which means that $\difFunc^{\prime}(\mNrm_{t})$ from~\eqref{eq:difFuncDecreases} is guaranteed to be negative if
\begin{align}
  \RNrmByGRnd \equiv (\Rfree/\PermGroFac) & < 1  \labelsafe{eq:FHWCFails}.
\end{align}
...
A footnote in~Section~\ref{sec:convergedcfunc} mentions reasons why it may be useful to calculate $\Ex_{t}[\log (\mLvl_{t+1}/\log \mLvl_{t})]$.
Here we show that one way of doing that is to calculate a nonlinear adjustment factor for the expectation of the growth factor.
\begin{align*}
\log \left(\mLvl_{t+1}/\mLvl_{t}\right) & = \log (\PermGroFac \permShk_{t+1} \mNrm_{t+1})- \log \mNrm_{t} 
\\ & = \log \PermGroFac (\aNrm_{t}\RNrmByGRnd+\permShk_{t+1}\tranShk_{t+1})- \log \mNrm_{t} 
\\ & = \log \PermGroFac (\aNrm_{t}\RNrmByGRnd+1+(\permShk_{t+1}\tranShk_{t+1}-1))- \log \mNrm_{t}
\end{align*}

Now define $\breve{\mNrm}_{t+1}=\aNrm_{t}\RNrmByGRnd+1$, and compute the expectation:
\begin{align*}
  \Ex_{t}[\log \left(\mLvl_{t+1}/\mLvl_{t}\right)]
   & = \Ex_{t}\left[\log \PermGroFac (\breve{\mNrm}_{t+1}+(\permShk_{t+1}\tranShk_{t+1}-1))\right]- \log \mNrm_{t} 
\\ & = \log \PermGroFac + \Ex_{t}\left[\log \breve{\mNrm}_{t+1}\left(1+(\permShk_{t+1}\tranShk_{t+1}-1)\breve{\mNrm}_{t+1}^{-1}\right)\right]- \log \mNrm_{t} 
\\ & = \underbrace{\log \PermGroFac + \log \breve{\mNrm}_{t+1} - \log \mNrm_{t}}_{\equiv \log \Ex_{t}[\mLvl_{t+1}/\mLvl_{t}]} + \Ex_{t}\left[ \log (1+(\permShk_{t+1}\tranShk_{t+1}-1)\breve{\mNrm}_{t+1}^{-1})\right]
\end{align*}
and exponentiating tells us that
\begin{align}\labelsafe{eq:mBalLogVSmBalLvl}
  \exp(\Ex_{t}[\log \mLvl_{t+1}/\mLvl_{t}]) & = \Ex_{t}[\mLvl_{t+1}/\mLvl_{t}]\exp( \Ex_{t}\left[ \log (1+(\permShk_{t+1}\tranShk_{t+1}-1)\breve{\mNrm}_{t+1}^{-1})\right])
\end{align}
and this latter factor is a number that approaches 1 from below as $\mNrm_{t}$ rises.
Thus the expected growth rate of the log is smaller than the log of the growth rate of the expected growth factor.
This implies that the $\mNrm$ at which `balanced growth' can be expected in the log, $\mBalLog$, exceeds the corresponding point for the ratio, $\mBalLvl$.

Furthermore, in the limit as $\mLvl_{t}$ gets arbitrarily large, if the {\RIC} holds and thus $\MPCmin > 0$, $\aNrm_{t+1}$ rises without bound, as does $\breve{\mNrm}_{t+1}=\aNrm_{t+1}\RNrmByGRnd+1$, so the approximation $\log (1+\epsilon) \approx \epsilon$ becomes arbitrarily good.
Consequently, the last term on the RHS of \eqref{eq:mBalLogVSmBalLvl} can be approximated as
\begin{align*}
  \Ex_{t}\left[ \log (1+(\permShk_{t+1}\tranShk_{t+1}-1)\breve{\mNrm}_{t+1}^{-1})\right]) & \approx \Ex_{t}\left[ (\permShk_{t+1}\tranShk_{t+1}-1)\breve{\mNrm}_{t+1}^{-1})\right])
                                                                                            \\ & = 0
\end{align*}
...
\begin{equation}\begin{split}
  \BalGroFac{\mFunc}_{t+1}(\aNrm) % = \Ex_{t}[\BalGroFac{\aNrm}\RNrmByGRnd+\tranShk_{t+1}\permShk_{t+1} ] \\
   & = 1+\aNrm\RNrmByGRnd
\\  \TargetNrm{\mFunc}_{t+1}(\aNrm)% & = \Ex_{t}[\TargetNrm{\aNrm}(\RNrmByGRnd/\permShk_{t+1})+\tranShk_{t+1}\permShk_{t+1} ] \\
 & = 1+\aNrm\underbrace{\RNrmByGRnd/\InvEPermShkInv}_{\bar{\RNrmByGRnd} > \RNrmByGRnd}
...
%  \\  \mTrgNrm - \mBalLvl & =1+ (\mTrgNrm-\cFunc(\mTrgNrm))\RNrmByGRnd/\InvEPermShkInv - (1+(\mBalLvl-\cFunc(\mBalLvl))\RNrmByGRnd)
 & = \left(\TargetNrm{\aNrm} \InvEPermShkInv^{-1} - \BalGroFac{\aNrm} \right)\RNrmByGRnd 
\\ & = \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - {\aNrm}(\mBalLvl) \right)\RNrmByGRnd 
\\ & = \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - \left({\aNrm}(\mTrgNrm+\mBalLvl-\mTrgNrm)\right) \right)\RNrmByGRnd 
\\ & \approx \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - \left({\aNrm}(\mTrgNrm)+(\mBalLvl-\mTrgNrm)\aFunc^{\prime}(\TargetNrm{\mNrm})\right) \right)\RNrmByGRnd 
%  \\ & = \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - {\aNrm}(\mTrgNrm)-(\mBalLvl-\mTrgNrm){\aNrm}^{\prime} \right)\RNrmByGRnd
%  \\ & = \left({\aNrm}(\mTrgNrm) \InvEPermShkInv^{-1} - {\aNrm}(\mTrgNrm)+(\mTrgNrm-\mBalLvl){\aNrm}^{\prime} \right)\RNrmByGRnd
  \\ (\mTrgNrm - \mBalLvl)(1-\underbrace{{\aNrm}^{\prime}(\mTrgNrm)\RNrmByGRnd}_{< \GPFacRaw < 1}) & = (\InvEPermShkInv^{-1} - 1)\TargetNrm{\aNrm}\RNrmByGRnd
%\\ & = \left((\mTrgNrm-\cFunc(\mTrgNrm)) \InvEPermShkInv^{-1} - (\mBalLvl-\cFunc(\mTrgNrm+\mBalLvl-\mTrgNrm)) \right)\RNrmByGRnd 
%\\ & = \left((\mTrgNrm-\cFunc(\mTrgNrm)) \InvEPermShkInv^{-1} -( (\mBalLvl-\cFunc(\mTrgNrm)+(\mBalLvl-\mTrgNrm)\MPC)) \right)\RNrmByGRnd 
\end{split}\end{equation}
The RHS of this equation is strictly positive because $\InvEPermShkInv^{-1}>1$ and both $\TargetNrm{\aNrm}$ and $\RNrmByGRnd$ are positive; while on the LHS, $(1-\RNrmByGRnd\aNrm^{\prime})>0$.
So the equation can only hold if $\TargetNrm{\mNrm}-\BalGroFac{\mNrm} > 0$.
That is, the target ratio exceeds the pseudo-steady-state ratio.\footnote{The use of the first order Taylor approximation could be substituted, cumbersomely, with the average of $\aFunc^{\prime}$ over the interval to remove the approximation in the derivations above.}
...
\\      \Ex_{t}[\log \left(\aNrm(\mNrm_{t})\Rfree+\permShk_{t+1}\tranShk_{t+1}\PermGroFac\right)] & = \log \PermGroFac \mNrm_{t}
\\      \Ex_{t}[\log \left(\aNrm(\mNrm_{t})\RNrmByGRnd+\permShk_{t+1}\tranShk_{t+1}\right)] & = \log \mNrm_{t}
\end{split}\end{equation}
...
\begin{equation}\begin{split}
    \exp(\Ex_{t}[\log \left(\aNrm(\BalGroRte{\mNrm})\RNrmByGRnd+\permShk_{t+1}\tranShk_{t+1}\right)]) - (\aNrm(\BalGroFac{\mNrm})\RNrmByGRnd+1 ) = \BalGroRte{\mNrm} - \BalGroFac{\mNrm}
\end{split}\end{equation}

Now we use the fact that the expectation of the log is less than the log of the expectation, 
\begin{equation}\begin{split}
%    \log \Ex_{t}[\aNrm(\BalGroRte{\mNrm}_{t})\RNrmByGRnd+\permShk_{t+1}\tranShk_{t+1}] & <     \log \left(\aNrm(\BalGroRte{\mNrm}_{t})\RNrmByGRnd+1\right) \\
 \exp(\Ex_{t}[\log \left(\aNrm(\BalGroRte{\mNrm})\RNrmByGRnd+\permShk_{t+1}\tranShk_{t+1}\right)]) & < \left(\aNrm(\BalGroRte{\mNrm})\RNrmByGRnd+1    \right)
\end{split}\end{equation}
so
\begin{equation}\begin{split}
    \exp(\Ex_{t}[\log \left(\aNrm(\BalGroRte{\mNrm})\RNrmByGRnd+1\right)]) - (\aNrm(\BalGroFac{\mNrm})\RNrmByGRnd+1 ) < \BalGroRte{\mNrm} - \BalGroFac{\mNrm}
\\    \left(\aNrm(\BalGroRte{\mNrm})\RNrmByGRnd+1\right) - (\aNrm(\BalGroFac{\mNrm})\RNrmByGRnd+1 ) < \BalGroRte{\mNrm} - \BalGroFac{\mNrm}    
\\     (\aNrm(\BalGroRte{\mNrm})-\aNrm(\BalGroRte{\mNrm}+\BalGroFac{\mNrm}-\BalGroRte{\mNrm}))\RNrmByGRnd& < \BalGroRte{\mNrm} - \BalGroFac{\mNrm}
\\     (\aNrm(\BalGroRte{\mNrm})-(\aNrm(\BalGroRte{\mNrm})+(\BalGroFac{\mNrm}-\BalGroRte{\mNrm})\bar{\aFunc}^{\prime})\RNrmByGRnd& < \BalGroRte{\mNrm} - \BalGroFac{\mNrm}
\\     (\BalGroRte{\mNrm}-\BalGroFac{\mNrm})\bar{\aFunc}^{\prime}\RNrmByGRnd& < \BalGroRte{\mNrm} - \BalGroFac{\mNrm}
\\     \underbrace{\bar{\aFunc}^{\prime}\RNrmByGRnd}_{< \GPFacRaw} & < 1
\end{split}\end{equation}
...
Due to the model's nonlinearities the values of $\mNrm$ at which the expected growth rate of $\cLvl$ matches $\PermGroFac$ is very slightly different from the $\mNrm$ at which the growth rate at which expected growth of $\mLvl$ is $\PermGroFac$.
Defining $\grave{\mNrm}$ as the $\mNrm$ at which $\Ex_{t}[\cLvl_{t+1}/\cLvl_{t}]=\PermGroFac$, we can show that to first order $\grave{\mNrm} \approx \BalGroFac{\mNrm}.$
\begin{align*}
  \Ex_{t}[\cFunc(\mNrm_{t+1})\permShk_{t+1}] & = \cFunc(\mNrm_{t}) \labelsafe{eq:balgrostableC}.
