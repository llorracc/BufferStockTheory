#!/usr/bin/env python3
"""concepts_render.py — render concepts/*.yml to _ai/concepts/*.md.

For each `concepts/<id>.yml` we emit `_ai/concepts/<id>.md` with:
...
Also emits `_ai/concepts/index.md` — a single-page table of contents
...
SRC = ROOT / "concepts"
OUT = ROOT / "_ai" / "concepts"
PAPER_URL = "../../BufferStockTheory.md"  # relative from _ai/concepts/<id>.md
...
def render_concept(concept: dict, concept_ids: set[str]) -> str:
    ...
    if bd:
        ...
        out.append("")
        if perch == "(deferred)" and stage == "(deferred)":
            out.append(
                "> *Reserved field — the bellman-ddsl perch/stage mapping is "
                "**deferred** for every concept in this atlas; the cross-repo "
                "bridge has not been authored. This is not a delivered "
                "correspondence.*"
            )
            out.append("")
