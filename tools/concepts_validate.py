...
Checks (in order):
  ...
  8. no line-number citations in gloss / notes / english (cite a stable
     anchor instead — line numbers break across re-renders)
...
# Brittle line-number citations: "line 1438", "lines 2317-2318",
# "BufferStockTheory.md:1438". Glosses must cite stable anchors instead.
LINE_CITATION_RE = re.compile(r"\blines?\s+\d+|\.md:\d+", re.IGNORECASE)
...
def main() -> int:
    ...
    for stem, c in concepts.items():
        ...
        # 8. glosses/notes cite stable anchors, never brittle line numbers
        prose = []
        if isinstance(c.get("gloss"), str):
            prose.append(("gloss", c["gloss"]))
        if isinstance(defeq, dict) and isinstance(defeq.get("english"), str):
            prose.append(("defining_equation.english", defeq["english"]))
        for ri, rel in enumerate(rels):
            if isinstance(rel, dict) and isinstance(rel.get("note"), str):
                prose.append((f"relations[{ri}].note", rel["note"]))
        for field_name, text in prose:
            m = LINE_CITATION_RE.search(text)
            if m:
                failures.append(
                    f"{path}: {field_name} has a line-number citation ({m.group(0)!r}) — "
                    f"cite a stable anchor (eq-…, prop-…, lemma-…, claim-…) instead"
                )
