 % Store the tex for standalone compilation
 \ifthenelse{\boolean{Web}}{
\begin{figure}[tbp] % Web
\centerline{\includegraphics[width=0.7\linewidth]{\econtexRoot/Figures/cFuncsConverge}} % Web 
\caption{Convergence of the Consumption Rules} % Web 
\label{fig:cFuncsConverge} % Web 
\end{figure} % Web
}{ % Web - no
\begin{figure}[tbp]
\centerline{\includegraphics[width=5.25in]{\econtexRoot/Figures/cFuncsConverge}}
\caption{Convergence of the Consumption Rules}
\label{fig:cFuncsConverge}
\end{figure}
} % Web - no
