...
\newcommand{\thankstext}{
  The paper's results \href{https://econ-ark.github.io/nbreproduce}{can be automatically reproduced} using the {\ARKurl} toolkit, which can be cited per our references (\cite{carroll_et_al-proc-scipy-2018}); for reference to the toolkit itself see \href{https://econ-ark.org/acknowledging}{Acknowledging Econ-ARK}.
...
That is, not all Bewley models with fluctuating income exhibit buffer stock saving, and not all models that exhibit buffer stock saving satisfy the mathematical assumptions that guarantee boundedness of marginal marginal utility imposed by Schectman or Bewley (and inherited by almost all of the subsequent literature through to the recent contributions of \cite{mstIncFluct,maUnboundedDP}). %%% AS TO AS make sure there is an implicit link made explicity via a hyperlink to the theoery section.
...
%%% Note to selves: maybe think about restoring this somewhere
...
In the existing literature going all the way back to \cite{fisherInterestTheory},\footnote{\cite{fisherInterestTheory} in Ch. IV, Section 3 states "I shall treat the two terms (impatience and time preference) as synonymous".}, time preference `$\DiscFac$' and patience have often been treated as synonymous. %%% Note to selves: Krusell and Smith (1998) is one such other sinner; uses 'patience' to refer to the level of \DiscFac? I have not seen anyone use the term impatience condition. \beta is refereed to as "impatience" and sometimes \beta =1/(1+p) and p is the "rate of impatience". `Exotic preferences for macroeconomists' by Backus et al (2004) says \beta<1 is 'impatience'.
...
%% As \cite{harmenbergInvariant} points out, a stationary distribution of market resources, weighted by permanent income still exists under growth impatience. %% CDC to AS: Is this true? %%% %%% AS to CDC: I have not proved this myself. This was a statement in the BST original? AS to do this as part of category diagram. 
...
%% AS TO CDC: can we move the following footnote? Or place it in the abstract?
%% \footnote{The paper's insights are instantiated in the {\ARKurl} toolkit, whose \href{https://hark.readthedocs.io/en/stable/reference/ConsumptionSaving/ConsIndShockModel.html}{buffer stock saving module} flags parametric choices under which a problem is degenerate or under which stable ratios of wealth to income may not exist.} %% AS TO AS: make sure exact about parameteric choices is made in the title or abstract. 
...
Traditional Bellman approaches to showing existence rely on assumptions that guarantee the boundedness of utility and marginal utility~\citep{slpMethods}.\footnote{Our CRRA utility function does not satisfy Bewley's assumption that $\uFunc(0)$ is well-defined, or that $\uP(0)$ is bounded above. %%% Note to selves: Bewley says $\uFunc(0)=0$ and $\uP(0)$<1 See Ass. 2.5 in Bewley.
...
%  For a related continuous-$t$ model, see~\cite{tocheUrisk}.
  ...
  \cite{mnUnique} assume a framework with compact action sets and real-valued utility which cannot handle relative risk aversion greater than 1. %%% AS to check this
  
  Two approaches do allow relative risk aversion greater than 1:  A literature employing time iteration operators defined by Euler equations~\citep{deatonLiqConstr, lsIncFluct, mstIncFluct}, and one that employs transformations of the Bellman equation \citep{rinconZapatero2024}, but in all of these cases an artificial borrowing constraint is present (or its moral equivalent, as in Bewley). %%% AS to double check mst to confirm
}.
