...
%% AS to CDC: I edited the above from the text below, which was the original before my pass of the appendix. Keen for above to be a road-map of the section.
...
%% [x] FVAC is really about conusmption paths along path of strictly positive income shocks? Explain this somwhere in the paper.  CDC2AS: No, strictly positive _permanent income_ which requires strictly positive permanent shocks $\permShk > 0$ which we impose.  I think it IS explained somewhere, right?
...
%% AS to CDC: Terminology. Should we say "normalized market resources" above? Are you OK with the terminology consistency?

%% AS to CDC: Chirs, can you please do a very solid check of the terminology in this section (Section 3)? IMPORTANT
...
%% Can we remove the comment below? I think it was also commented in the original paper. 
\begin{comment}
...
%% AS to CDC: can we delete the below? 
