...
**MyST HTML edition (canonical online):** <https://llorracc.github.io/BufferStockTheory/> — the rendered, browsable single-page version of the paper. Republished from this repo via `cd ../BufferStockTheory-make && make myst-publish-bare-from-latest`. Each publish leaves a `published/YYYYMMDD-HHMMSSZ` tag on the source-side commit so previously-deployed states are recoverable.
